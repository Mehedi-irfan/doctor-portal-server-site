...
async function verifyToken (req, res, next) {
  if(req.headers?.authorization?.startsWith('Bearer ')){
    ...
    try{
        const decoededUser = await admin.auth().verifyIdToken(token);
        req.decodedEmail = decoededUser.email;
    }
    catch{
...
async function run () {
    try{
        ...
        const appointmentCollection = database.collection('appoitment');
      ...
      app.put('/appointments/:id', async (req, res) => {
        ...
        app.put('/users/admin', verifyToken, async (req, res) => {
          ...
          if(requester){
            ...
            res.status(401).json({message : 'you do not have access to make a admin'})
          }
